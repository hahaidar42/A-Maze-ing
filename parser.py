...
def parse_coords(value: str) -> Tuple[int, int]:
    ...
    try:
        ...
        raise ValueError(f"invalid coordinate format, expected 'x,y' got '{value}'") from error
...
def parse_config(filepath: str) -> Dict[str, Union[int, str, Tuple[int, int], bool, Optional[int]]]:
    ...
    if not (0 <= entry[0] < width and 0 <= entry[1] < height):
        raise ValueError(f"Entry {entry} out of bounds for {width}x{height} maze")
    
    # Check exit bounds
    if not (0 <= exit_coords[0] < width and 0 <= exit_coords[1] < height):
        raise ValueError(f"Exit {exit_coords} out of bounds for {width}x{height} maze")
